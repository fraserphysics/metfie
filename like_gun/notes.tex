...
\end{equation*}
In my notation for experiments and models, the parameters are $c$ and
...
\end{align*}
I will take the first term as an approximation of the Fisher
Information for the following reasons:
\begin{enumerate}
\item If the model fits the data well, the term $(y-\mu(c))$ will be
  small where the expectation has weight.
\item Since the code must provide estimates of the components of the
  first term as part of the optimization procedure, it comes for
  free.
\item The number of simulations necessary to estimate
  $\frac{\partial^2 \mu(c)}{\partial c^2}$ using finite differences is
  $\frac{n_c(n_c+1)n_y}{2}$.
\end{enumerate}
