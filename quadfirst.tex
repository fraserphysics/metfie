...
\onecolumn
\begin{landscape}
  I used the python sympy package to calculate moments of the images
analytically.  Here is the expected value of $h$ in the image of
$z\equiv(x_0,y_0)\equiv(h_0,g_0)$
\begin{equation*}
  \Ex
\end{equation*}
and here is the expected value of $g$ 
\begin{equation*}
  \Ey
\end{equation*}
The expressions for the elements of the covaraince matrix are even
worse.  Although I find the expressions incomprehensible, I believe
that they are correct.  The ellipses in Figure~\ref{fig:eric}
represent the moments in terms of the level sets
\begin{equation*}
  (z-\mu)^T \Sigma^{-1} (z-\mu) = 2
\end{equation*}
for the image sets of four initial points $z_0$.  I used sympy
functions to substitute floating point values in for variables in the
analytic expressions for the moments to draw the ellipses in the
figure.
\begin{figure}
  \centering
