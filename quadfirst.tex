...
\begin{abstract}
  ...
  \textbf{To do:}
  \begin{itemize}
  \item Figure out if conjecture in Section~\ref{sec:speculation}
    implies convergence, and make alternative plans if not
  \item Rewrite Sections~\ref{sec:integral-equations} and~\ref{sec:approximate}
    using notation developed in Section~\ref{sec:scaling} and
    $\Delta_y=1$
  \item Delete Section~\ref{sec:solve}
...
\end{align*}
the ratio $d = \frac{u}{\Delta_y^2}$ is the number that characterizes
the problem and it is clear that the strucure of allowed sequential
pairs near $\tilde g=d$ depends only on the difference $d-\tilde g$
and not on $d$ itself.  Without loss of generality, in this section, I
assume $\Delta_y=1$ and let $d$ define the problem.  I will use $h$
and $g$ as variables rather than $\tilde h$ and $\tilde g$.
...
\newcommand{\pair}{\beta}
\newcommand{\triple}{\gamma}
\newcommand{\Skip}{\delta}
...
\item[$\allowed$] Indicator function
  \begin{equation*}
    ...
    \end{cases}
  \end{equation*}
\item[$\PFop$, $\eval$, $\refunc$] The Peron Frobenius operator, its
  eigenvalue and its right eigenfunction defined by
  \begin{equation*}
    \PFop(\refunc)(z) = \int \refunc(z') \allowed(z',z) dz' = \eval \refunc(z).
  \end{equation*}
...
\item[$\triple$] The joint probability density function for sequential
  triples of points
\item[$\Skip$] The joint denisty of a pair separated by a skipped
  value, ie,
  \begin{equation*}
    \Skip(z_a,z_b) \equiv \int \triple(z_a,z,z_b) dz.
  \end{equation*}
...
\item[$\preimage$] The pre-image function
  ...
  \end{equation*}
\item[$\scale$] A rescaling obtained by dividing $\Delta_y$ in half, ie,
  replacing $d$ with $4d$.
\item[$z', \allowed',\cdots, \preimage'$] Corresponding objects for
  rescaled problem, eg, if $z=(h,g)$ then
  \begin{equation*}
    z' = \scale(z) = (2h, 4g).
  \end{equation*}
  Note that $\allowed' = \allowed$ except that $\allowed'$ operates on
  a larger domain.
\end{description}

\subsubsection{Consistiency}
\label{sec:consistiency}

In the limit as $\lim d \rightarrow \infty$
\begin{equation}
  \label{eq:consistent}
  \pair(z_a,z_b) = \Skip'(z_a',z_b')
\end{equation}

First show that the allowed transitions in one step at the original
scale are the same as the allowed transiitons in two steps at the
finer primed scale, ie,
\begin{equation}
  \label{eq:composition}
  \allowed(z_a,z_b) =
  \begin{cases}
    1 & \text{if } \exists z :\,\allowed'(z_a',z) \allowed'(z,z_b') \\
    0 & \text{otherwise}
  \end{cases}
\end{equation}
which is equivalent to
\begin{equation*}
  \scale \circ \preimage = \preimage'^2 \circ \scale
\end{equation*}
Then from
\begin{align}
  \triple(z_a, z, z_b) &= \frac{\pair(z_a,z_b)\pair(z_b,z_c)}{\int
                         \pair(z,z_b) dz} \nonumber \\
  \Skip(z_a, z_b) &= \int \frac{
                    \refunc(z_a) \allowed(z_a,z)
                    \lefunc(z) \refunc(z) \allowed(z,z_b) \lefunc(z_b)
                    }
                    { \int \lefunc(z) \allowed(z,z_b) \refunc(z_b) dz}
                    d z \nonumber \\
  &= \frac{1}{\eval} \int \lefunc(z_a) \allowed(z_a, z) \allowed(z,
    z_b) \refunc(z)b) dz
\end{align}
conclude that combined with \eqref{eq:composition}
\begin{equation}
  \label{eq:sufficient}
  \lefunc'(z') = \sqrt{\eval'} \lefunc(z)  
\end{equation}
is sufficient.
...
In addition to \eqref{eq:consistent} the stationary distribution must
converge to a limit.
