...
allowed point in ${\cal G} \times {\cal H}$.  In thinking about that
calculation, I noticed that there is only one parameter in the problem.
While both $\Delta_y$ and $u$ appear in \eqref{eq:UL_g} and
\eqref{eq:UL_h}, in the alternative coordinates in which $\Delta_y=1$,
ie,
\begin{align*}
  \tilde g(g) &= \frac{g}{\Delta_y^2} \\
  \tilde h(h) &= \frac{h}{\Delta_y}
\end{align*}
it is clearer that the behavior near $\tilde g=d$ does not depend on
$\Delta_y$, and that the ratio $d = \frac{u}{\Delta_y^2}$ is the
number that characterizes the problem.

So, without loss of generality, in this section, I assume $\Delta_y=1$
and let $d$ define the problem.  The domain of the problem is the
set of points
\begin{equation*}
  D = \left\{ z = (g,h) :
    \begin{array}{l}
...
the upper/lower edge of the wedge with the boundary respectively.
...
single lines appear in Eqn.~\eqref{eq:sympy}.  However,
Figure~\ref{fig:eric} suggests that the expressions are correct.  The
ellipses in the figure are the level sets
\begin{equation*}
  \left\{ z : (z-\mu)^T \Sigma^{-1} (z-\mu) = 2 \right\}
\end{equation*}
for the image sets $P(z_0)$ of four initial points $z_0$.  I used
sympy functions to substitute floating point values in for variables
in the analytic expressions to draw the ellipses.
\onecolumn
\begin{landscape}
\begin{figure}
  \centering
...
the eigenfunction away from the big $g$ region converges as
