...
\newcommand{\leftfunction}{\rho}
\newcommand{\rightfunction}{\tilde \rho}
...
\begin{itemize}
\item Calculate other eigenfunction and the stationary probability
\item Make subscripts on $\leftfunction$, $\rightfunction$ and
  $\lambda$ correspond to $\frac{1}{n}$ or $\tau$ consistently
\item Plot $\lambda(\tau)$
\item Code that calculates conditional distribution after $n$ steps of
  $\Delta t/n$
\end{itemize}
...
\end{align*}
Now drop $\lambda$ from $\leftfunction_\lambda$ and calculate:
\begin{align*}
  \lambda \leftfunction(g) &= \int_0^1 \leftfunction(h)
                             \Aindicate(h,g) dh \\
  \lambda \leftfunction(g) &= \int_0^1 \leftfunction(h)
                             dh ~~\forall g
                             \leq \tau \\
  &= \text{ some constant } \\
  \leftfunction(g) &= a ~~\forall g \leq \tau \\
  \lambda \leftfunction(g) &= \int_{g-\tau}^1 \leftfunction(h) dh ~~\forall
  g > \tau\\
  \lambda \frac{\partial}{\partial g} \leftfunction_\lambda(g) &=
            - \leftfunction_\lambda(g-\tau)~~\forall g > \tau \\
  \lambda \leftfunction(g) &= a - \int_0^{g-\tau} \leftfunction(h) dh ~~\forall
  g > \tau\\
\end{align*}

If $\tau = 1$ then the image of every function $\function$ is a
multiple of the constant function $\leftfunction(g) = 1$ and the
eigenvalue is $1$.  Also there is a one-dimensional subspace
corresponding to $\lambda=1$ and an infinite dimensional subspace
...
and
\begin{align*}
  \lambda \leftfunction (g) &= \left( \leftfunction \Aop \right) (g) \\
  \lambda a &= a \left( \frac{1}{2} + \int_{\frac{1}{2}}^1
...
If $\tau = \frac{1}{n}$, then $\leftfunction$ is piecewise
...
\begin{equation}
  ...
  \leftfunction_\infty(g) \equiv \lim_{n \rightarrow \infty}
  \leftfunction(g) = e^{-g},
\end{equation}
and
\begin{equation}
  \label{eq:limit_value}
  \lambda_\infty = \int_0^1 \leftfunction_\infty(h) dh = 1 - \frac{1}{e}.
\end{equation}
Figure~\ref{fig:eigenfunctions} illustrates $\leftfunction_n$ for a
few values of $n$.
...
I begin by analyzing a process with an countable number of values for
...
be non-negative with normalization that give the
stationary distribution for that process as
\begin{equation}
  \label{eq:stationary}
...
The principal task of this paper is to determine if the limit in the
continuous time version of \eqref{eq:n_step}, ie,
\begin{equation}
  \label{eq:2}
  \density(g_t|g_0) = \lim_{n \rightarrow \infty}  \frac{\delta(g_0)
    \left( \Aop_\tau \right)^{n-1} \Aindicate_\tau(\cdot,g_n)
    \rightfunction_\tau(g_n)} { \left( \lambda_\tau \right)^n
    \rightfunction_\tau(g_0)} \text{ with } \tau=\frac{t}{n},
\end{equation}
exists.  Since the analysis in Section~\ref{sec:continuous_g} leading to
Equations~\eqref{eq:limit_function} and \eqref{eq:limit_value} gives
the value $\lim_{\tau \rightarrow 0} \lambda_\tau = 1 - \frac{1}{e}$
and suggests that $\lim_{\tau \rightarrow 0} \rightfunction_\tau$
exists\marginpar{calculate $\rightfunction_\infty$.}, all that remains
is to show that
\begin{equation}
  \label{eq:3}
  T_t \equiv \lim_{n \rightarrow \infty} \left(
    \frac{\Aop_{\frac{t}{n}}}
